...
namespace VikingCrew.Tools.UI {
    public class SpeechBubbleManager : MonoBehaviour
    {
        ...
        [SerializeField]
        private List<SpeechbubblePrefab> _prefabs = null;
        ...
        private IEnumerator DelaySpeechBubble(float delay, Vector3 position, string text, SpeechbubbleType type, float timeToLive, Color color)
        {
            yield return new WaitForSeconds(delay);
            AddSpeechBubble(position, text, type, timeToLive, color);
        }
        ...
        public void AddDelayedSpeechBubble(float delay, Vector3 position, string text, SpeechbubbleType type = SpeechbubbleType.NORMAL, float timeToLive = 0, Color color = default(Color))
        {
            StartCoroutine(DelaySpeechBubble(delay, position, text, type, timeToLive, color));
        }
